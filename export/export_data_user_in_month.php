...
$sql = "
    SELECT lr.id, ui.first_name, ui.last_name, lr.fromDate, lr.toDate, lr.total_days, lr.reason, lr.status, lr.date_send,
           lr.approved_by, d.department_name, lr.updated_at, lr.comment
...
// Title and styling
$sheet->setCellValue('A1', 'របាយការណ៍សំណើច្បាប់ប្រចាំខែ') // Monthly Leave Request Report
    ->mergeCells('A1:L1');
...
$i = 3; // Start from the third row to accommodate the headers
...
foreach ($monthlyRequestsResult as $request) {
    $sheet
        ...
        ->setCellValue('I' . $i, $request['approved_by']) // Approved By
...
$filename = 'monthly_leave_requests_report.xlsx';

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($leaveId) || empty($comment)) {
        $_SESSION['alert'] = ['type' => 'danger', 'message' => 'តម្រូវឱ្យមានលេខសម្គាល់សំណើបញ្ឈប់ និងមតិយោបល់សម្រាប់ការបដិសេធ។'];
    ...
    try {
        ...
        $updateSql = "UPDATE leave_requests SET status = 'បដិសេធ', comment = :comment, approved_by = :approved_by, updated_at = NOW() WHERE id = :id";
        $stmt = $conn->prepare($updateSql);
        $stmt->execute(['comment' => $comment, 'approved_by' => $approvedBy, 'id' => $leaveId]);

        // Check if any rows were affected
        if ($stmt->rowCount() > 0) {
            $_SESSION['alert'] = ['type' => 'success', 'message' => 'សំណើត្រូវបានបដិសេធ។'];
        } else {
            $_SESSION['alert'] = ['type' => 'warning', 'message' => 'មិនមានការផ្លាស់ប្តូរណាមួយទេ។ សូមពិនិត្យលេខសម្គាល់វិញ។'];
        }
    } catch (PDOException $e) {
        // Handle SQL errors
